...
* local_db: Shows how to use the library to synchronize ram and disk at any frequency, and from more than one thread simultaneously. Suitable for saving game data, program checkpoints, or even e-commerce orders, anything as long as the database is managed by a single process.
...
You may even separate more. There is no limit to the number of databases the library can simultaneously handle.